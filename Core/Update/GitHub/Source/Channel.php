...
class Channel
{
    ...
    public function getChannelRelease( $Download = false )
    {
        ...
        if ($this->Config->getChannelActiveRelease()) {
            if (false === ( $ReleaseList = Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                    'channel' )->getCacheData( sha1( $this->Config->getChannelListRelease() ) ) )
            ) {
                ...
                Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                    'channel' )->setCacheData( $ReleaseList, sha1( $this->Config->getChannelListRelease() ) );
            }
    ...
    private function downloadTagTree( Release $Release )
    {
        ...
        if (false === ( $TagList = Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                'channel' )->getCacheData( sha1( $this->Config->getChannelListNightly() ) ) )
        ) {
            $TagList = json_decode(
            ...
            Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__, 'channel' )->setCacheData( $TagList,
                sha1( $this->Config->getChannelListNightly() ) );
        }
        foreach ((array)$TagList as $TagItem) {
            ...
            if ($Tag->getVersion()->getVersionString() == $Release->getVersion()->getVersionString()) {
                $Release->setTag( $Tag );
                if (false === ( $TreeList = Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                        'channel' )->getCacheData( sha1( $this->Config->getGitHubChannelTree( $Tag->getIdentifier() ) ) ) )
                ) {
                    ...
                    Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                        'channel' )->setCacheData( $TreeList,
                        sha1( $this->Config->getGitHubChannelTree( $Tag->getIdentifier() ) ) );
                }
    ...
    private function downloadUpdate( Release $Release )
    {
        ...
        foreach ((array)$BlobList as $Blob) {
            ...
            if (false === ( $Data = Api::groupCore()->unitCache()->apiFile( $this->DataCache, __METHOD__,
                    'update' )->getCacheData( $Blob->getIdentifier() ) )
            ) {
                $Data = json_decode(
                ...
                Api::groupCore()->unitCache()->apiFile( $this->DataCache, __METHOD__, 'update' )->setCacheData( $Data,
                    $Blob->getIdentifier() );
            }
    ...
    public function getChannelPreview( $Download = false )
    {
        ...
        if ($this->Config->getChannelActivePreview()) {
            if (false === ( $ReleaseList = Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                    'channel' )->getCacheData( sha1( $this->Config->getChannelListPreview() ) ) )
            ) {
                $ReleaseList = json_decode(
                ...
                Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                    'channel' )->setCacheData( $ReleaseList, sha1( $this->Config->getChannelListPreview() ) );
            }
            foreach ((array)$ReleaseList as $ReleaseItem) {
                if ($ReleaseItem->prerelease) {
                    $Release = $this->Type->buildRelease( $ReleaseItem );
    ...
    public function getChannelNightly( $Download = false )
    {
        ...
        if ($this->Config->getChannelActiveNightly()) {
            if (false === ( $ReleaseList = Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                ...
                Api::groupCore()->unitCache()->apiFile( $this->ChannelCache, __CLASS__,
                    'channel' )->setCacheData( $ReleaseList, sha1( $this->Config->getChannelListNightly() ) );
            }
            foreach ((array)$ReleaseList as $ReleaseItem) {
                $Release = $this->Type->buildRelease( $ReleaseItem );
                /**
